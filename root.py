...
from fastapi import FastAPI, Query, Path, Body, Header, Response, status, Form, File, UploadFile, HTTPException, Request
...
item_htttpException = {"foo": "The Foo Wrestlers"}

@app.get("/httpException/{item_id}")
async def read_item(item_id: str):
  ...
  return {"item": item_htttpException[item_id]}
...
@app.get("/validation_exception/{item_id}")
async def read_item(item_id: int):
  if item_id == 3:
